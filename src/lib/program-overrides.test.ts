...
describe('the week read back after a refused save', () => {
	...
	// a save answering with the write path's prose and nothing marked on the block
	// that caused it. The week is read again, and the server's fresh account is
	// what marks the blocks.
	...
	const ws1 = { savedRowID: 'ws-1' };

	it('marks the row the coach has not moved since the week was read', () => {
		// Which is the case the ticket was opened for: the week was read clean, the
		// training moved under the loaded page, and the coach touched nothing. The
		// row the save sent and the row the server judged are one row, so the
		// server's account of it is about the value on screen exactly.
		const held = [{ item_id: 'a', overrides: { rest_seconds: 90 } }];
		expect(markedFromReread(held, week([refusedRow]), ws1)).toEqual([
			{
				item_id: 'a',
				overrides: { rest_seconds: 90 },
				override_stale: true,
	...
	it('leaves a row the coach has moved to the rule that already judges it', () => {
		// stale_fields describes the row the server stores and the refusal is about
		// the row the save sent, and here they are two different rows: nothing has
		// judged 120. So the row is left as it stands, its own marking included, for
		// standingRefusals to weigh, and the strip above carries the refusal in the
		// write path's own words.
		const moved: SessionOverride[] = [
			{
				item_id: 'a',
				overrides: { rest_seconds: 120 },
				override_stale: true,
				stale_fields: [{ field: 'rest_seconds', reason: 'an older read' }]
			}
		];
		expect(markedFromReread(moved, week([refusedRow]), ws1)).toEqual(moved);
		const plain = [{ item_id: 'a', overrides: { rest_seconds: 120 } }];
		expect(markedFromReread(plain, week([refusedRow]), ws1)).toEqual(plain);
	});

	it('hands back the row the coach holds and not the row the server sent', () => {
		// The two ask the same thing, which is what lets the refusal be written onto
		// this one, and they are not the same object: the server's carries a row id
		// and writes its fields in its own order. What the coach reads afterwards
		// has to be what they typed.
		const held = [{ item_id: 'a', overrides: { cycles: 4, rest_seconds: 90 } }];
		const stored: SessionOverride = {
			...refusedRow,
			overrides: { rest_seconds: 90, cycles: 4 }
		};
		const marked = markedFromReread(held, week([stored]), ws1);
		expect(marked[0].overrides).toEqual({ cycles: 4, rest_seconds: 90 });
		expect(marked[0].id).toBeUndefined();
	...
	it('names only the fields the row on screen carries', () => {
		...
		const held = [{ item_id: 'a', overrides: { rest_seconds: 90 } }];
		expect(markedFromReread(held, week([spread]), ws1)[0].stale_fields).toEqual([
			{ field: 'rest_seconds', reason: REASON }
		]);
	});

	it('drops a refusal naming nothing the row carries', () => {
		// The refusal is attributed to the stored row, so the field name is the only
		// bridge to what is on screen. With none of them carried there is nothing to
		// point at, and marking the row as a whole would quote a refusal about a
		// value this week does not set.
		const carriesNeither: SessionOverride = {
			...refusedRow,
			overrides: { cycles: 4 },
			stale_fields: [{ field: 'rest_seconds', reason: REASON }]
		};
		const held = [{ item_id: 'a', overrides: { cycles: 4 } }];
		expect(markedFromReread(held, week([carriesNeither]), ws1)).toEqual([
	...
	it('reads a refusal the server attributed to nothing as one about the row', () => {
		...
		const held = [{ item_id: 'a', overrides: { rest_seconds: 90 } }];
		const marked = markedFromReread(held, week([unattributed]), ws1);
		expect(marked[0].override_stale).toBe(true);
	...
	it('replaces the marking a read before the save left behind', () => {
		...
		// takes was out of date, so where the fresh read does judge a row it
		// replaces all of it rather than being merged into it. A row the server
		// still holds unchanged and no longer refuses loses its mark.
		const held = [
			{
				item_id: 'b',
				overrides: { reps: 4 },
		...
		const stored = week([refusedRow, { id: 'row-2', item_id: 'b', overrides: { reps: 4 } }]);
		expect(markedFromReread(held, stored, ws1)).toEqual([{ item_id: 'b', overrides: { reps: 4 } }]);
	});

	it('keeps every marking on a session the server holds no row for', () => {
		// A session the save sends as a new row: one the coach has just added, one
		// dragged in from another week, one Duplicate week copied, and every
		// unlocked session of a week the "does not belong to this week" recovery
		// cleared the ids of. The server holds nothing under it, so the read was
		// never asked about it, and that is not the same answer as the read
		// refusing nothing of it. Reading it as the second is how a week comes back
		// from a refused save with less marked on it than it had before.
		const held: SessionOverride[] = [
			{
				item_id: 'a',
				overrides: { rest_seconds: 90 },
				override_stale: true,
				stale_fields: [{ field: 'rest_seconds', reason: 'the read the week was loaded with' }]
			}
		];
		expect(markedFromReread(held, week([refusedRow]), { savedRowID: undefined })).toEqual(held);
		expect(markedFromReread(held, week([refusedRow]), { savedRowID: 'ws-9' })).toEqual(held);
	});

	it('answers whether the read holds the session at all', () => {
		// Which is what tells the caller apart a session the read judged and found
		// clean from one it was never asked about, and the two cannot be told apart
		// from the marked rows alone.
		const reread = rereadWeek([{ id: 'ws-1', overrides: [] }]);
		expect(rereadHolds(reread, ws1)).toBe(true);
		expect(rereadHolds(reread, { savedRowID: 'ws-9' })).toBe(false);
		expect(rereadHolds(reread, { savedRowID: undefined })).toBe(false);
	});
	...
	it('keys the rows by the session the server holds them under', () => {
		...
		const reread = rereadWeek([
			...
			{ id: 'ws-2', overrides: [{ id: 'row-2', item_id: 'a', overrides: { rest_seconds: 90 } }] }
		]);
		const held = [{ item_id: 'a', overrides: { rest_seconds: 90 } }];
		expect(markedFromReread(held, reread, ws1)[0].override_stale).toBe(true);
		expect(
			markedFromReread(held, reread, { savedRowID: 'ws-2' })[0].override_stale
		).toBeUndefined();
	});
	...
	it('hands the marking to the same reader the modal already runs on', () => {
		...
		const trees = trainingTrees(base, emptyGridLayouts());
		const held = [{ item_id: 'a', overrides: { rest_seconds: 90 } }];
		const marked = markedFromReread(held, week([refusedRow]), ws1);
		expect(staleRefusalLines(marked[0], base[0])).toEqual([{ fields: 'rest', reason: REASON }]);
		const elsewhere = [{ item_id: 'a', overrides: { rest_seconds: 90, cycles: 4 } }];
...
function openModalOn(training: TrainingItem[], stored: SessionOverride[]) {
	...
	const declaredLayouts = declaredGridLayouts(declared, stored);
	// The week's row is merged onto the training written out in the layout that
	...
	const layouts = withOpenedLayouts(declaredLayouts, items);
...
describe('the layout a week sends', () => {
	...
	describe('a layout the coach chose on a grid that read as one row', () => {
		...
		// A row the server takes: one load per row the training declares, and the
		// same load in every one of them. The collapse the editor performs is the
		// week's own here, which is the only collapse there is since
		// Krakoer/crimpy#101: the row is merged onto the training written out in the
		// layout the row declares, so a week whose loads varied would be read as
		// varying and there would be no single row to pick a layout on.
		const stored: SessionOverride[] = [
			{ item_id: 'grid', overrides: { loads: Array.from({ length: 8 }, () => kg(20)) } }
		];
		...
		it('goes out as the coach set it, in the layout they picked', () => {
			// Eight rows of the one load they were reading, laid out again because
			// they asked for it, which is theirs to ask. It is what the week stored
			// as well, and that is the point: the collapse they picked a layout on
			// was of the week's own values, so laying the grid out again cannot lose
			// one.
			const opened = openModalOn(gridTraining(flat), stored);
			pickLayout(opened.items[0], 'set');
			const { sent } = sentNow(opened);
	...
	describe('a layout the coach chose on a grid the server refused', () => {
		const REFUSED_REASON = 'loads holds 12 entries but the granularity declares 8 rows';
		const flat = Array.from({ length: 8 }, () => kg(10));

		// Twelve loads written when the block ran three sets, against the eight rows
		// it declares now, all of them the same load. The rows the item still has
		// all read that one load, so the editor reads the week back as one row, and
		// the four the training dropped are what the server refuses it for.
		const stored: SessionOverride[] = [
			{
				item_id: 'grid',
	...
	describe('a layout the coach chose that the training already prescribes', () => {
		...
		// One load per row the training declares, every one of them the load the
		// training prescribes, so the coach opens on a row reading exactly what the
		// training says. It has to be all four of them: a row asking for anything
		// else is a prescription of this week's, which Krakoer/crimpy#101 is about
		// and which this is not.
		const stored: SessionOverride[] = [
			{ item_id: 'grid', overrides: { loads: Array.from({ length: 4 }, () => kg(20)) } }
		];
	...
	// Krakoer/crimpy#101: a week's grid varying over a training whose own grid does
	// not. The row is written against the rows of the layout it declares, and it
	// used to be merged onto the tree the editor reads, which the training's own
	// coinciding values had already collapsed to a single row. Eight values landed
	// on one row and seven of them were gone: not hidden, gone, since the diff the
	// request is built from reads the merged tree and nothing else.
	//
	// The two cases are told apart by nothing the coach can see, which is why both
	// are here. Where the week's first load differs from the training's, a row is
	// shown and it is truncated. Where it coincides, the diff comes out empty: no
	// badge, a footer saying the week runs the training as it is written, and an
	// apply that sends no row at all and drops what the week held.
	describe('a week whose grid varies over a training whose own grid does not', () => {
		const flat = Array.from({ length: 8 }, () => kg(10));

		// Eight loads, no two alike, one per row the training declares: a row the
		// server takes and never marks, so there is nothing on screen to warn the
		// coach that what they are reading is not what the week holds.
		function varyingWeek(first: number): SessionOverride[] {
			return [
				{
					item_id: 'grid',
					overrides: {
						loads: [kg(first), kg(21), kg(22), kg(23), kg(24), kg(25), kg(26), kg(27)]
					}
				}
			];
		}

		// Every rep of every set, as the athlete would play it: the week's row
		// merged onto the training as it is written, which is the prescription the
		// coach typed and the one thing the modal may not move.
		function prescription(overrides: SessionOverride[]) {
			const item = mergeOverrides(gridTraining(flat), overrides)[0];
			const played = [];
			for (let set = 0; set < 2; set++) {
				for (let rep = 0; rep < 4; rep++) played.push(storedConfig(item, set, rep));
			}
			return played;
		}

		describe('storing a first load the training does not prescribe', () => {
			const stored = varyingWeek(20);

			it('is shown as the eight loads the week wrote, not as the first of them', () => {
				const { asOpened } = openModalOn(gridTraining(flat), stored);
				expect(asOpened.openedOnScreen[0].overrides.loads).toEqual(stored[0].overrides.loads);
			});
			...
			it('prescribes what the week prescribed, rep for rep', () => {
				const { sent } = sentNow(openModalOn(gridTraining(flat), stored));
				expect(prescription(sent)).toEqual(prescription(stored));
			});
		});

		describe('storing a first load the training does prescribe', () => {
			const stored = varyingWeek(10);

			it('is shown as the eight loads the week wrote, rather than as nothing at all', () => {
				const { asOpened } = openModalOn(gridTraining(flat), stored);
				expect(asOpened.openedOnScreen).toHaveLength(1);
				expect(asOpened.openedOnScreen[0].overrides.loads).toEqual(stored[0].overrides.loads);
			});

			it('is a customisation of this week, which is what the coach is told', () => {
				// The badge on the block and the count in the footer read off the row
				// the week will hold. An empty one here is the modal saying this week
				// runs the training as it is written, over a week that asks for seven
				// loads it does not.
				const { sent } = sentNow(openModalOn(gridTraining(flat), stored));
				expect(sent.map((override) => override.item_id)).toEqual(['grid']);
			});

			it('goes back asking exactly what the week stored, rather than nothing', () => {
				// The row the apply sends. It used to be dropped whole, and
				// keepStoredGridArrays could not put the loads back: nothing was
				// emitted for it to substitute into.
				const { sent } = sentNow(openModalOn(gridTraining(flat), stored));
				expect(sent[0].overrides).toEqual(stored[0].overrides);
				expect(sent[0].overrides.loads).toHaveLength(
					declaredRows(gridTraining(flat)[0], sent[0].overrides)
				);
			});
		...
		// The other half of Krakoer/crimpy#99, which this leaves alone: the collapse
		// of an item whose own values coincide is what the coach reads, and a week
		// whose eight loads agree is still the one row they call for.
		it('is still one row where the loads of the week itself coincide', () => {
			const stored: SessionOverride[] = [
				{ item_id: 'grid', overrides: { loads: Array.from({ length: 8 }, () => kg(25)) } }
			];
			const { asOpened } = openModalOn(gridTraining(flat), stored);
