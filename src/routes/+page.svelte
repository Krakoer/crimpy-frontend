    ...
    <div class="border-2 border-black bg-white">
      <div class="flex border-b-2 border-black">
        <button
          ...
          style="font-family: monospace; font-size: 13px; background-color: {activeTab === 'login' ? '#C6613F' : 'white'}; color: {activeTab === 'login' ? 'white' : 'black'};"
        ...
        <button
          class="flex-1 py-3 px-4 font-medium transition-colors"
          style="font-family: monospace; font-size: 13px; background-color: {activeTab === 'register' ? '#C6613F' : 'white'}; color: {activeTab === 'register' ? 'white' : 'black'};"
      ...
      <div class="p-6">
        {#if error}
          <div class="mb-4 p-3 border border-red-600 bg-red-50" style="font-family: monospace; font-size: 12px; color: #B85450;">
            {error}
          </div>
        {/if}
        ...
        {#if activeTab === 'login'}
          <form onsubmit={(e) => { e.preventDefault(); handleLogin(); }}>
            <div class="mb-4">
              <label class="block mb-2 font-medium" style="font-family: monospace; font-size: 12px; color: #666; letter-spacing: 0.5px;">
                EMAIL
              </label>
              <input
                bind:value={email}
                required
                class="w-full px-3 py-2 border border-black bg-white"
            ...
            <button
              type="submit"
              disabled={loading}
              class="w-full py-3 px-4 font-medium transition-opacity"
              style="font-family: monospace; font-size: 13px; background-color: #C6613F; color: white; opacity: {loading ? 0.6 : 1};"
            >
